...
namespace Px.Dcat
{
    public class DataCollector
    {
        ...
        private void loadSettings(DcatSettings dcatSettings)
        {
            ...
            string languageMappingFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Languages.json");
            _languageMapping = JsonReader.ReadDictionary(languageMappingFile);
        ...
        private void addRecursive(Item item, List<PxMenuItem> path, List<Dataset> d)
        {
            ...
            else if (item is TableLink)
            {
                ...
                try
                {
                    IPXModelBuilder builder = _fetcher.GetBuilder(table.ID.Selection);
